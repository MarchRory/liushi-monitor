import {
  ...
  DEFAULT_TEMPORARY_BREADITEM_LIMIT,
...
export abstract class BaseBreadCrumb {
  ...
  constructor({
    ...
    const { tabbar_urls, ignore_urls = DEFAULT_BREADCRUMB_CONFIG.ignore_urls } =
      options || {};
    this.deviceInfo = this.getDeviceInfo();
    this.ignoredUrls = new Set(ignore_urls);
    this.tabbarUrls = new Set(tabbar_urls);
    this.initBrowserUnloadListener();
    this.debouncedPushRecord = debounce(
      (...args: Parameters<typeof this.pushRecord>) => this.pushRecord(...args),
    );
    this.debouncedPopRecord = debounce(
      (...args: Parameters<typeof this.popRecord>) => this.popRecord(...args),
    );
  ...
  pushRecord(routeInfo: IBaseRouteInfo) {
    ...
    // 兼容tabbar界面
    // 路径栈中, 当前栈顶页面路径
    const prePathRecord = lastestRecord
      ? lastestRecord.stack[lastestRecord.stack.length - 1]
      : null;
    let stack: string[] = [];
  ...
  popRecord(routeInfo: IBaseRouteInfo) {
    ...
    Promise.resolve().then(() => {
      ...
      lastestRecord.page_exposure =
        lastestRecord.leave_time - lastestRecord.enter_time;
  ...
  private reportRecord() {
    ...
    const sendData: IBaseTransformedData<"userBehavior", "page_exposure"> = {
  ...
  private initBrowserUnloadListener() {
    window.addEventListener("beforeunload", () => this.saveToIndexDB(), {
      capture: true,
    });
  }
  /**
