...
export class BaseClient<T extends MonitorTypes = MonitorTypes>
  implements IBaseClient
{
  ...
  constructor(initialOptions: ISDKInitialOptions & { VueApp?: App }) {
    ...
    aop(window.history, "pushState", (nativeFn: History["pushState"]) =>
      this.globalPushAndReplaceAOP(nativeFn, this.eventBus),
    );
    aop(window.history, "replaceState", (nativeFn: History["replaceState"]) =>
      this.globalPushAndReplaceAOP(nativeFn, this.eventBus),
    );
    aop(window, "onpopstate", (nativeFn: Window["onpopstate"]) =>
      this.globalPopStateAOP(nativeFn, this.eventBus),
    );
    aop(
      document,
      "onvisibilitychange",
      (nativeFn: Document["onvisibilitychange"]) =>
        this.globalVisibiityChangeAOP(nativeFn, this.eventBus),
    );
    aop(window, "onclick", (nativeFn: Window["onclick"]) =>
      this.globalClickEventAOP(nativeFn, this.eventBus),
    );
    aop(window, "onerror", (nativeFn: Window["onerror"]) =>
      this.globalJsSyncErrorAOP(nativeFn, this.eventBus),
    );
  }
  /**
   * 注册插件+
  ...
  reportProcessWapper(currentPlugin: IBasePlugin<MonitorTypes, any>) {
    return async (originalData: IPluginTransportDataBaseInfo) => {
      ...
      if (hooks.onDataCollected) {
        customCollectedData = await hooks.onDataCollected?.call(
          this,
          currentPlugin.indicatorName,
          originalData,
        );
      }

      // 格式化收集到的数据
      let transformedData = currentPlugin.dataTransformer?.call(
        this,
        this,
        customCollectedData,
      );
      if (hooks.onDataTransformed) {
        transformedData = await hooks.onDataTransformed?.call(
          this,
          currentPlugin.indicatorName,
          transformedData,
        );
      ...
      currentPlugin.dataConsumer?.call(
        this,
        this.baseTransport,
        transformedData,
      );
  ...
  private globalPushAndReplaceAOP(
    nativeFn: History["pushState"] | History["replaceState"],
    eventBus: typeof this.eventBus,
  ) {
    return function (this: History, ...args: Parameters<typeof nativeFn>) {
      Promise.resolve().then(
        throttle(() => eventBus.notify("onPushAndReplaceState", args)),
      );
      return nativeFn.apply(this, args);
    };
  }
  private globalPopStateAOP(
    nativeFn: Window["onpopstate"],
    eventBus: typeof this.eventBus,
  ) {
    return function (
      this: WindowEventHandlers,
      args: typeof nativeFn extends null ? any[] : PopStateEvent,
    ) {
      Promise.resolve().then(
        debounce(() => eventBus.notify("onPopState", args), 100),
      );
      if (nativeFn) {
        return nativeFn.apply(this, [args]);
      }
  ...
  private globalVisibiityChangeAOP(
    nativeFn: Document["onvisibilitychange"],
    eventBus: typeof this.eventBus,
  ) {
  ...
  private globalClickEventAOP(
    nativeFn: Window["onclick"],
    eventBus: typeof this.eventBus,
  ) {
  ...
  private globalJsSyncErrorAOP(
    nativeFn: Window["onerror"],
    eventBus: typeof this.eventBus,
  ) {
    return function (
      this: GlobalEventHandlers,
      ...args: Parameters<OnErrorEventHandlerNonNull>
    ) {
