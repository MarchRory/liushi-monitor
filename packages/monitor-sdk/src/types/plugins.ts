...
import { BaseEventTypes, IBaseBreadCrumbItem, IBaseTransformedData, IOriginalData, MonitorTypes } from '../types'
...
export interface IBasePlugin<T extends MonitorTypes, E extends BaseEventTypes<T>> {
     ...
     * @returns 
     */
    dataTransformer: (client: BaseClient<T>, originalData: IPluginTransportDataBaseInfo<E>) => IBaseTransformedData<T, E>
    /**
     * 消费处理后的数据, 进行附加数据添加、上报服务器等操作
     * @param transformedData 格式化后的数据
     * @returns 
     */
    dataConsumer: (transport: BaseTransport, transformedData: IBaseTransformedData<T, E>) => void
}
