...
class Vue3AppMonitorClient extends BaseClient {
  ...
  spaStartLoadTimiing(path: string) {
    ...
    if (
      !isUndefined(spaPerformanceConfig?.ignoredPageUrls) &&
      spaPerformanceConfig.ignoredPageUrls.includes(path)
    )
      return;
  ...
  async sendSpaLoadPerformance() {
    ...
    const sendData: IBaseTransformedData<
      "performance",
    ...
    > = {
      ...
      collectedData: {
        ...
        data: {
          value: +(
            loadedTime - this.spaPagePerformanceRecord.beforeSpaChange
          ).toFixed(),
        },
      },
    };
