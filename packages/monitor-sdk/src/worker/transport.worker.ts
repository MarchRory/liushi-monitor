...
const reportDataMap = new Map<
  RequestBundlePriorityEnum,
  IProcessingRequestRecord<SendDataTextType>[]
>([
 ...
 * 一些需要通信完成初始化的自定义配置
 */
const customConfig: Map<
  WorkerConfigKey,
  TransportWorkerConfig[WorkerConfigKey]
> = new Map();
...
function postMessageToMainThread<
  T extends WorkerPostToMainThreadEvent = WorkerPostToMainThreadEvent,
>(order: { type: T; payload: IWorkerPostToMainThreadMessage[T] }) {
...
function getNextData(): IProcessingRequestRecord<"ciphertext"> | null {
  const bundleData: IProcessingRequestRecord<SendDataTextType> = {
    textType: "ciphertext",
    data: [],
  ...
  for (const priority of PRIORITY_ORDER) {
    ...
    while (
      queue &&
      queue.length &&
      bundleData.data.length < singleMaxReportSize
    ) {
  ...
  if (debugMode === false) {
    bundleData.data = [
      encrypt(JSON.stringify(bundleData.data), encryptionConfig),
    ];
  }
  return bundleData;
}
...
async function loadNextReportTask() {
  ...
  asyncTaskScheduler.addTask(
    () => (type?: TransportTaskRunType) => sendWithRetry(nextData, type),
  );
  workerEventBus.notify("onPrepareNextReport");
}
...
function preLoadTask({
  ...
  priority,
  textType = "plaintext",
}: IPreLoadParmas<MonitorTypes, BaseEventTypes, SendDataTextType>) {
  ...
  reportDataMap
    .get(priority)
    ?.push({ priority, textType, data, retryRecord: 0 });
...
function initTaskScheduler({
  reportTaskSizeLimit,
}: Pick<TransportWorkerConfig, "reportTaskSizeLimit">) {
...
self.addEventListener(
  "message",
  (ev: MessageEvent<ThreadMessage<"MainThread">>) => {
