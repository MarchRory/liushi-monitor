...
import {
  BaseErrorOverviewSearch,
  BaseSpecificErrorTable,
} from '../dto/base.dto';
...
export class ErrorAnalysisService {
  ...
  async calculateErrorOverview(dto: BaseSpecificErrorTable) {
    ...
    // 按时间分组的错误数
    const errorsByTime = await this.groupErrorsByTime(
      startTime,
      endTime,
      baseWhere,
    );
    ...
    // 按错误类型分组
    const errorsByType = await this.groupErrorsByType(
      startTime,
      endTime,
      baseWhere,
    );
    ...
    // 按URL分组
    const errorsByUrl = await this.groupErrorsByUrl(
      startTime,
      endTime,
      baseWhere,
    );
  ...
  async calculateSpecificErrorList(dto: BaseSpecificErrorTable) {
    ...
    const processedErrors = errors.map((error) => {
      try {
        const codeLocation = error.codeLocation
          ? JSON.parse(error.codeLocation)
          : {};
    ...
    return responseBundler(
      ...
      listBundler(result.total, result.list),
    );
  }

  // 按时间分组统计错误数量
  private async groupErrorsByTime(
  ...
  // 按错误类型分组
  private async groupErrorsByType(
    startTime: string | Date,
    endTime: string | Date,
    where: object,
  ) {
  ...
  // 按URL分组
  private async groupErrorsByUrl(
    startTime: string | Date,
    endTime: string | Date,
    where: object,
  ) {
