...
export class HttpAnalysisService {
  ...
  async calculateHttpOverView(dto: BaseHttpOverviewSearch) {
    ...
    // 生成Redis缓存键
    const cacheKey = `http_overview:${url}:${interfaceUrl}:${new Date(
      startTime,
    ...
    const hourlyDistribution = (await this.prismaService.$queryRaw(
      query,
    )) as IHttpOverview['hourlyDistribution'];
    ...
    const result: IHttpOverview = {
      ...
      methodDistribution: methodDistribution.map((item) => ({
        ...
        percentage: parseFloat(
          ((item._count.method / totalRequests) * 100).toFixed(2),
        ),
      })),
      statusCodeDistribution: statusCodeDistribution.map((item) => ({
        ...
        percentage: parseFloat(
          ((item._count.responseCode / totalRequests) * 100).toFixed(2),
        ),
      })),
      interfaceUsage: interfaceUsage.map((item) => ({
        ...
        percentage: parseFloat(
          ((item._count.id / totalRequests) * 100).toFixed(2),
        ),
      })),
      hourlyDistribution,
      slowRequests: {
