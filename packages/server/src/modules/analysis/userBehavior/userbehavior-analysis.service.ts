...
export class UserhehaviorAnalysisService {
  private readonly logger: Logger = new Logger(
    UserhehaviorAnalysisService.name,
  );
  constructor(
  ...
  async calculateMeaningfulUrls() {
    ...
    return responseBundler(
      ...
      listBundler(options.length, options),
    );
  }
  async calculateBehaviorOverview(dto: BaseBehaviorOverviewSearch) {
    ...
    const [viewsData, interactionData, exposureData, trendData] =
      await Promise.all([
  ...
  private async getViewsAnalysis(dto: BaseBehaviorOverviewSearch) {
    ...
    // 计算上一个时间段的数据用于环比
    const timeSpan = dayjs(dto.endTime).diff(
      dayjs(dto.startTime),
      'millisecond',
    );
    const previousStartTime = dayjs(dto.startTime).subtract(
      timeSpan,
      'millisecond',
    );
  ...
  private async getInteractionAnalysis(dto: BaseBehaviorOverviewSearch) {
    // 获取当前时间段的交互数据
    const currentPeriodInteractions =
      await this.prismaService.interaction.count({
        where: {
          url: dto.url,
          timestamp: {
            gte: new Date(dto.startTime),
    ...
    const previousPeriodInteractions =
      await this.prismaService.interaction.count({
        where: {
          url: dto.url,
          timestamp: {
  ...
  private async getExposureAnalysis(dto: BaseBehaviorOverviewSearch) {
    ...
    const timeSpan = Math.floor(
      dayjs(dto.endTime).diff(dayjs(dto.startTime), 'second'),
    );
    ...
    return {
      ...
      previousAvgValue: +(
        Math.floor(previousPeriodExposures._avg.value || 0) / 1000
      ),
    };
  }
  ...
  async getFunnelAnalysis(dto: BaseBehaviorOverviewSearch) {
    ...
    // 过滤掉没有曝光数据的路径栈
    const validPathStacks = pathStacks.filter(
      (stack) => stack.exposures.length > 0,
    );
    ...
    // 转换为数组格式
    const pathTransitions = Array.from(pathMap.entries())
      .map(([path, count]) => {
    ...
    for (let level = maxLevel; level >= 0; level--) {
      ...
      currentLevelPages.forEach((page) => {
        // 获取从该页面出发的所有路径
        const outgoingPaths = pathTransitions.filter(
          (item) => item.from === page,
        );
        ...
        const totalOriginalOutflow = outgoingPaths.reduce(
  ...
  async getExposureDepthAnalysis(dto: BaseBehaviorOverviewSearch) {
    ...
    // 计算每个路径的平均曝光时间
    const pathAvgExposure = pathExposureTime
      .map((path) => {
        const totalValue = path.exposures.reduce(
          (sum, exp) => sum + exp.value,
          0,
        );
        const avgValue =
          path.exposures.length > 0 ? totalValue / path.exposures.length : 0;
